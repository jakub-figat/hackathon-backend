...
async def get_verified_user(user: UserSchema = Depends(get_request_user)) -> UserSchema:
    if not user.is_verified:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You need to verify your account")
