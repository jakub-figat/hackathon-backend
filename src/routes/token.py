...
async def login(
    ...
    try:
        ...
        response.set_cookie(
            ...
            expires=(datetime + dt.timedelta(minutes=settings.refresh_expiration_time)).strftime(
                "%a, %d %b %Y %H:%M:%S GMT"
            ),
            httponly=True,
        )
        response.set_cookie(
            ...
            expires=(datetime + dt.timedelta(minutes=settings.token_expiration_time)).strftime(
                "%a, %d %b %Y %H:%M:%S GMT"
            ),
            httponly=True,
        )
