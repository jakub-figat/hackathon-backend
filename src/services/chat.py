...
class ChatService:
    ...
    async def get_chat_list(
        ...
        condition = (
            ...
            if filter_params.status is None
            else and_(ChatRequestModel.status == filter_params.status, ChatRequestModel.requested_id == user_id)
        )
    ...
    async def update_chat_status(
        ...
        chat_request = await self._session.scalar(
            select(ChatRequestModel).where(ChatRequestModel.id == chat_id, ChatRequestModel.requested_id == user_id)
        )
