...
class TicketService:
    ...
    async def get_ticket(self, ticket_id: UUID) -> TicketSchema:
        ...
        ticket = await session.scalar(
            select(TicketModel)
            .options(selectinload(TicketModel.services))
    ...
    async def get_volunteers(self, ticket_id: UUID, user_id: UUID) -> UserList:
        session = self._ticket_data_access._session
        ticket = await session.scalar(
            select(TicketModel).where(
                and_(
        ...
        requesters = await session.scalars(select(ChatRequestModel).where(ChatRequestModel.requested_id == user_id))
        requesteds = await session.scalars(select(ChatRequestModel).where(ChatRequestModel.requester_id == user_id))
        user_ids = [requester.requester_id for requester in requesters] + [
            requested.requested_id for requested in requesteds
        ]

        return UserList(users=user_ids)
